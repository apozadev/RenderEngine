...
#define __REFLECT_STRUCT_BEGIN(TYPE, PARENT_TYPE_EXP, CREATE_EXP) \
...
#define REFLECT_STRUCT_BASE_BEGIN(TYPE) \
  __REFLECT_STRUCT_BEGIN(TYPE, nullptr, return owner_ptr<void>(Factory::Create<TYPE>().release());)
...
#define REFLECT_STRUCT_BEGIN(TYPE, PARENT_TYPE) \
  __REFLECT_STRUCT_BEGIN(TYPE, &PARENT_TYPE::GetReflection(), return owner_ptr<void>(Factory::Create<TYPE>().release());)
