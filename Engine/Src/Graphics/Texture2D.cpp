...
public:
  ...
  Impl(void* _pData, uint32_t _uWidth, uint32_t _uHeight, ImageFormat _eFormat, unsigned int _uMipLevels, unsigned int _uMsaaSamples)
  {    
    m_pAPITexture = api::CreateAPITexture(_pData, _uHeight, _uHeight, _eFormat, _uMipLevels, _uMsaaSamples);
  }
  ...
  : Resource(_iBinding, _eStage)
{
  ...
  m_pImpl = std::make_unique<Impl>(rImage.m_pData, rImage.m_iWidth, rImage.m_iHeight, rImage.m_eFormat, _uMipLevels, _uMsaaSamples);
}
...
Texture2D::Texture2D(const Image& _rImage, int _iBinding , PipelineStage _eStage, unsigned int _uMipLevels, unsigned int _uMsaaSamples)
  : Resource(_iBinding, _eStage)
{  
  m_pImpl = std::make_unique<Impl>(_rImage.m_pData, _rImage.m_iWidth, _rImage.m_iHeight, _rImage.m_eFormat, _uMipLevels, _uMsaaSamples);
}

Texture2D::Texture2D(uint32_t _uWidth, uint32_t _uHeight, ImageFormat _eFormat, int _iBinding, PipelineStage _eStage, unsigned int _uMipLevels, unsigned int _uMsaaSamples)
  : Resource(_iBinding, _eStage)
{
  m_pImpl = std::make_unique<Impl>(nullptr, _uWidth, _uHeight, _eFormat, _uMipLevels, _uMsaaSamples);
}
